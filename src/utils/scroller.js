...
export default class Scroller {
   ...
   * @param isAnimate {Boolean?false} Whether the scrolling should happen using an animation
  ...
  scrollTo(left, top, isAnimate, zoom = 1) {
    ...
    // Don't do animation when no change detected, still call publish to make sure
    ...
    // Publish new values
    if (!this._isTracking) {
      this._publish(left, top, zoom, isAnimate)
    }
  }
   ...
   * @param isAnimate {Boolean ? false} Whether to use animation
  ...
  zoomTo(level, isAnimate, originLeft, originTop, callback) {
    ...
    // Push values out
    this._publish(left, top, level, isAnimate)
  }
  ...
  _publish(left, top, zoom = 1, isAnimate = false) {
    ...
    if (isAnimate && this.options.animating) {
